...
shinyServer(function(input, output) {
  ...
  output$skills_prior <- renderPlot({
  ...
  output$skills_after <- renderPlot({
  ...
  output$sentimentbefore <- renderPlot({
    #we need to use the NRC sentiments
    summary_skills_prior_program %>%
      inner_join(get_sentiments("nrc")) %>%
      count(word, sentiment, sort=TRUE) %>%
  ...
  output$sentimentafter <- renderPlot({
    #we need to use the NRC sentiments
    summary_skills_during_program %>%
      inner_join(get_sentiments("nrc")) %>%
      count(word, sentiment, sort=TRUE) %>%
